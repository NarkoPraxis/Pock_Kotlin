...
class PrismSkin(override val renderer: PuckRenderer) : PuckSkin {
    ...
    override fun DrawScope.drawBody() {
        ...
        val edgeSw = renderer.strokePaint.strokeWidth

        val angleOffsetRad = angleOffset * (Math.PI.toFloat() / 180f)
        for (i in 0 until sides) {
            val a1 = (i * 360f / sides) * Math.PI.toFloat() / 180f + angleOffsetRad
            val a2 = ((i + 1) * 360f / sides) * Math.PI.toFloat() / 180f + angleOffsetRad
            path.reset()
        ...
        path.reset()
        for (i in 0 until sides) {
            val a = (i * 360f / sides) * Math.PI.toFloat() / 180f + angleOffsetRad

    ...
    override fun render(canvas: Canvas, renderer: PuckRenderer) {
        ...
        for (i in points.size - 1 downTo 0) {
            ...
            val color = when {
                ...
                charging -> if (strobe) Palette.cyclingPurple(renderer.frame) else Palette.cyclingPurple(renderer.frame + 6)
                strobe -> theme.primary
                else -> Palette.cyclingPurple(renderer.frame)
            }

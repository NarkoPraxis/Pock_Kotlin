...
class SpiralLaunch(theme: ColorTheme) : PaddleLaunchEffect(theme) {
    ...
    override fun drawChargingPaddle(canvas: Canvas, puck: Puck) {
        drawCross(canvas, puck, paddleX, paddleY, aimX, aimY, phase, chargeFillRatio, frame * 0.35f)
    }
    ...
    override fun drawStrikingPaddle(
        ...
        drawCross(canvas, puck, cx, cy, aX, aY, ph, if (sweet) 1f else if (overcharged) 0f else 1f, frame * 0.35f + progress * 3f)
    }
    ...
    private fun drawCross(
        ...
        val half = paddleHalfLength(puck) * 0.9f
        val pX = -aY
        val pY = aX
        val color = if (ph == ChargePhase.Overcharged) theme.secondary else theme.secondary
        bar.color = color
        bar.strokeWidth = paddleThickness(puck)
    ...
    override fun drawResidual(canvas: Canvas, puck: Puck, rx: Float, ry: Float, remaining: Float) {
        ...
        canvas.drawCircle(rx, ry, puck.radius * (1f + (1f - remaining) * 1.2f), bar)
        bar.alpha = 255
    }
}
